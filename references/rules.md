...
bugfix 的 gold 必须包含真实功能代码修复，至少改动 1 个功能 Go 文件，功能代码增删总行数至少 5 行。正式轨迹中模型最终补丁相对 G1 也必须独立达到同一最低值；gold 达标不能替代模型补丁达标。机器统计只计非 `*_test.go` 的 Go 文件，不计测试、README、Markdown、配置和交付文件；达到最低值后，文件数和增删行数只作为审查信息记录，不得靠拆文件、加注释或堆无效代码制造规模。
...
- [ ] gold 修复与模型最终补丁各自至少改动 1 个功能 Go 文件、功能代码增删总行数至少 5 行，且没有凑规模
