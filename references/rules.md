...
- `success_criteria` 必须覆盖：修复前红、修复完之后全绿、没有回归、只验证公开行为；表达方式错开（编号式、分号式、冒号式交替），不要每行同一骨架；只写红/绿/回归三件事，不解释原因、不写「不是...而是...」等 AI 词，语言按高中生口语。句式写连贯一点，参考「修复完之后 20 遍全绿、没有回归；回退 X 再次红了；只看公开行为」，少用「基线」「修完」「无回归」「再红」这类刻板短词。
