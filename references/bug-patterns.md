...
每个模式都按「跨模块协同失效」设计，目标是形成真实的运行时故障链；bugfix 的 gold 必须至少有 5 行功能代码增删，达到最低值后文件数和增删行数只作为修复形状参考，
...
- 触发过程应忠实覆盖复现所需顺序，故障跨至少 2 个相关模块/包；目标测试断言题面描述的主要用户可见结果。
...
- **gold 修复形状示例**：store 返回深拷贝并补齐读路径同步；service 改为快照迭代；必要时同步调整 worker/cache 的所有权边界。只修改故障链实际需要的文件。
...
- **gold 修复形状示例**：`Add` 移到 spawn 前；关闭所有权覆盖全部路径；错误发送和 collector 同时正确处理结果、错误与取消。只修改故障链实际需要的文件。
...
- **gold 修复形状示例**：adapter 保留类型与元数据；transaction 只在成功路径提交并合并错误；必要时同步调整 service/worker 的决策与幂等语义。
...
- **gold 修复形状示例**：构造函数统一初始化；接口返回显式 `nil`；涉及的写路径和校验层恢复一致的空值契约。
...
- **gold 修复形状示例**：入口层向下传正确 ctx；store 改为参数传递；涉及的 worker/client 遵守同一生命周期。只调整实际传播链。
...
- **gold 修复形状示例**：parser 脱离复用缓冲区；service 向异步任务交付不可变快照；按实际所有权边界调整 cache/exporter。
...
- **gold 修复形状示例**：缩短 defer 生命周期；合并 commit/rollback 错误；补齐真实错误路径的释放和审计时序。
...
- **gold 修复形状示例**：引入单调版本契约和稳定幂等键；在实际终态写入与读取路径拒绝旧版本。
...
- **gold 修复形状示例**：贯通 ctx、定义 channel 关闭所有权、取消后停止重试，并让 shutdown 等待真实任务集合。
...
- **gold 修复形状示例**：保留原错误并合并 rollback 错；只在 commit 后发布；按实际链路补齐幂等和事件版本语义。
...
- **gold 修复形状示例**：builder 使用局部对象原子发布；recovery 不返回半成品；下游只接收已验证对象并在错误后停止处理。
...
- **gold 修复形状示例**：明确 pool 对象所有权和清理函数；请求链只传递快照；异步消费者不持有已归还的复用对象。
